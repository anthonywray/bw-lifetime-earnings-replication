************
* SCRIPT: 3.3_build_data_with_kids.do
* PURPOSE: Process the data in preparation for robustness analysis with kids
************
...
if `earnings' {
	...
	*	duplicates so that we are back to one observation per person.
	...
	// Why are there observations with missing values here?
	...
	// Now that we're using complete counts for 1900 to 1940, slwt = 1 for these censuses
	...
	*	stable for each racesing*age in years 2017-2070.
	...
	save "analysis/processed/data/earnings/census_earnings_with_kids.dta", replace
