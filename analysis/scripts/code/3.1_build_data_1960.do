************
* SCRIPT: 3.1_build_data_1960.do
* PURPOSE: Process the data in preparation for robustness analysis including data from 1960
************
...
if `earnings' {
	...
	*	two representative samples.
	...
	// Separate out 1950 and post-1960 data since we don't need it to impute 1940 non-wage income
	...
	* earnings information. We will impute these values.
	...
	// First, impute non-wage income from 1960 to 1940, 
	...
	*	predictor.
	...
	tab age nowages if year == 1960, m
	
	reg nowages i.racesing i.age i.occ1950 i.ind1950 i.bpl i.statefip if year == 1960
	predict nowages_predicted if inlist(year,1940,1960)
	xtile nowages_predicted_bins = nowages_predicted if inlist(year,1940,1960), n(20)

	xtile incwage_bins = incwage if inlist(year,1940,1960), n(20)

	// Now predict non-wage income for 1940
	
	reg inc_nonwg i.nowages_predicted_bins i.incwage_bins i.racesing i.age ///
		i.occ1950 i.ind1950 i.bpl i.statefip if year == 1960
	predict inc_nonwg_predicted if inlist(year,1940,1960)
	...
	*	stable for each racesing*age in years 2017-2070.
	...
	save "analysis/processed/data/earnings/census_earnings_using_1960.dta", replace
